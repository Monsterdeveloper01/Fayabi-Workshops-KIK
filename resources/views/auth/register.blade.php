...
<head>
    ...
    <title>FAYABI - Daftar Member Baru</title>
    ...
    <main class="min-h-screen relative overflow-hidden flex items-center justify-center p-4">
        ...
        <section class="w-full max-w-5xl mx-auto relative z-10">
                ...
                <div class="flex flex-col md:flex-row">
                    ...
                    <div class="p-10 md:w-8/12 space-y-6">
                        ...
                        <form method="POST" action="{{ route('register') }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input type="text" name="name" value="{{ old('name') }}" required autofocus
                                        placeholder="Farel Yazid Bintang"
                                ...
                                <div class="space-y-2">
                                    <label class="flex items-center gap-2 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">
                                        <i class="fa-solid fa-envelope text-red-600"></i> Email Utama
                                    </label>
                                    <input type="email" name="email" value="{{ old('email') }}" required
                                        placeholder="dev@fayabi.com"
                                        class="w-full bg-white/5 border-2 border-white/10 text-white px-5 py-3.5 rounded-2xl text-sm font-bold focus:outline-none focus:border-red-600 transition-all placeholder:text-slate-700">
                                    @error('email')
                                        <span class="text-red-500 text-xs font-bold ml-1 block">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="space-y-2">
                                    <label class="flex items-center gap-2 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">
                                        <i class="fa-solid fa-shield-check text-red-600"></i> Konfirmasi
                                    </label>
                            ...
                            <div class="p-4 bg-red-600/5 rounded-2xl border border-red-600/10 mt-6">
                                <p class="text-[9px] text-slate-500 font-bold uppercase tracking-tight leading-relaxed">
                                    <i class="fa-solid fa-circle-info text-red-600 mr-1"></i> Dengan mendaftar, Anda menyetujui protokol keamanan sistem FAYABI Workshop.
                                </p>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row gap-4 pt-6">
                                <button type="submit" class="flex-[2] bg-red-600 hover:bg-red-700 text-white font-black py-4 rounded-2xl uppercase tracking-[0.2em] text-[11px] shadow-[0_10px_30px_rgba(220,0,46,0.3)] transition-all flex items-center justify-center gap-2 transform active:scale-95">
                                    KONFIRMASI DAFTAR <i class="fa-solid fa-user-plus"></i>
                                </button>
                                
                                <a href="{{ route('login') }}" class="flex-1 border-2 border-white/10 text-slate-400 hover:text-white hover:border-white/30 font-black py-4 rounded-2xl uppercase tracking-[0.2em] text-[10px] transition-all flex items-center justify-center text-center">
                                    KE LOGIN
                                </a>
                            </div>
            ...
            <p class="text-center mt-6 text-slate-700 text-[9px] font-black uppercase tracking-[0.4em]">
                Fayabi Workshop &copy; {{ date('Y') }} - Secure System Identification
            </p>
        </section>
    </main>
